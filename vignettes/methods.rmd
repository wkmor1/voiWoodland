...
### EVPXI
EVPXI requires a slightly more complicated algorithm. The EVWOI in equation 4, which is used to calculate EVPXI can be calculated as above. However, calculating the EVWPXI for the $i^{th}$ parameter(s) requires the double looping algorithm which we outline in the pseudo-code in Box 1.

******

#### Box 1: Calculating EVWPXI$_i$

To calculate EVPXI for any given parameter or paramters of interest, which we will denote as the $i^{th}$ parameter(s), we apply the following algorithm to the 375 alternative parameterizations of the 169 parameters.

...
\usepackage{url}
\usepackage[citestyle = authoryear, bibstyle = voiWoodland, natbib = true,
  maxcitenames = 3, maxbibnames = 999, uniquelist = false, sortcites = true,
  backend = bibtex]{biblatex}
\addbibresource{voiWoodland.bib}
\usepackage{lineno}
\linenumbers
...
@
\title{\huge{Value of information for woodland management: What you can do, tell's you what you need to know}\\
...
\section{Summary}
\begin{enumerate}
\item When managing in the face of uncertainty, monitoring or experimentation can facilitate decisions yielding greater expected benefits. But it is only rational to monitor or experiment if the expected net benefit of reducing uncertainty is positive.
\item A value of information analysis can reveal the expected benefit of reducing uncertainty. We performed value of information analyses for models of Box-Ironbark Forest and Woodland management in central Victoria, a complex system with high structural and parametric uncertainty. These analyses could help reveal whether to experiment and monitor to reduce uncertainty in a system model, which aspects of model uncertainty would be most important to address and what sampling strategies would be most cost-effective to undertake.
\item We found that the outcome of forest management was much more sensitive to some model parameters than others, and the greatest value of information tended to come from reducing uncertainty about the parameters to which the outcomes were most sensitive.
\item Among candidate sampling strategies, we found that those that achieved the greatest cost effectiveness focused monitoring on particular aspects of the system itself or particular management actions. We also found that the value of information was sensitive to the set of management options available to the decision-maker. As the number of options increased, the value of information for some parameters increased, while for others it remained more or less constant.
\item \textit{Synthesis and applications}.
Monitoring and experimentation to reduce uncertainty can lead to better decision making for vegetation management.  We demonstrate how to apply a value of information analysis for a complex, highly-parameterised system model. We show how such analyses can be performed for a continuous model of a decision problem. Our analysis shows it is probably more cost-effective to monitor certain aspects of the system and employ a targeted sampling design than monitor with a view to reducing uncertainty in all dimensions of model space simultaneously. We also found that the optimal focus for learning can change if the set of available management actions changes.
\end{enumerate}
\textbf{Keywords:} Bayesian decision analysis, cost-benefit analysis, thinning, expert elicitation, multivariate adaptive regression splines, sensitivity analysis, state and transition model, uncertainty.
\newpage

\section{Introduction}
Ecosystems are typically managed under uncertainty. Experimentation and monitoring can reduce uncertainty and facilitate management decisions with greater expected benefits. The decision to experiment or monitor at all is only rationally justifiable, however, if the expected benefit when making a decision with new information, rather than without, is greater than the cost of learning. Determining the value of information allows the calculation of this benefit \citep{Raiffa1968}. Value of information theory is a set of decision theoretic tools that have previously been applied to decision problems in economics, medicine, engineering and other domains \citep{Dakins1999, Yokota2004, Claxton2008, Wu2013}.

Sometimes it is unclear whether monitoring is necessary \citep{Mcdonaldmadden2010}. With the value of information toolset, a decision analyst can assess what to monitor, how much to monitor and even whether monitoring is justified at all. Compared with other fields such as operations research and medicine, ecology and natural resource management have been slower to adopt value of information analyses for decision making, although recently, some examples have begun to appear in the literature (e.g., \cite{Polasky2001, Moore2011, Runge2011, Runting2013}). Most examples have focused on the application of value of information to discrete model problems (e.g., \cite{Moore2012}). Furthermore, ecological decision problem solving has tended to focus on models of population dynamics \citep{Runge2011, Canessa2015, Johnson2014, Maxwell2015}. In contrast, communities or ecosystems have rarely been the focus of value of information analyses. This paper demonstrates how value of information analyses can be applied to a complex continuous system model to better manage an ecological community.
...
If we perform an experiment, $e$, such that we monitor 1000\,ha of BIFAW, and observe $m$ hectare transitions, then if $m > 0$ we eliminate the possibility that $p = 0$ , thus conclude $\phi = 0$ and update the posteriorprobability density function to
...
We prefer this formulation of the updating process over other possible methods of formualting a zero-inflated beta posterior, where the updater might learn about $p$ and $\phi$ simulatenously. We feel it reflects the perspective of the expert elicitor who has no prior information that would give them unequal belief in the experts. Obtaining new information shouldn't change the equality of expert knowledge, it should just change the belief of the experts themselves.
...
<<change_constraint_evpi, echo = FALSE>>==
...
We have also shown that if learning is targeted at subsets of the system, so as to update multiple parameters simultaneously, then some system states and management options would be better foci than others. This is because the uncertainty of some parameter sets is more critical to management decisions than others. Managers would be wiser to focus monitoring on areas subject to mangagement under the harvest management scenario rather than thinning or no management options, because learning yb reducing the uncertainty in the parameters associated with those latter options would not change the decision about which management option to use as much on average if their uncertainty was reduced. Also, if managers chose only to monitor one system state, then learning about transitions from the low-density regrowth state would see greater expected benefit than the other three states.
...
In the analyses we have presented here, we have not accounted for correlations among the transition probabilities. Experts that gave a particular value for one parameter were often more likely to have given a particular value for another parameter. We were unable to account for within-expert correlations among parameters when we updated them in the posterior analyses. The implication is that there may really be less feasible model space to update than is implied when assuming no correlations among parameters. Therefore the available space for learning and therefore potential value of information is lower than we have assumed. However, if learning about one parameter actually provides you with information about other parameters (because of the correlations), then value of information might be higher. Yet, it may be justified to ignore the expert driven correlation among parameters. An expert could be knowledgeable about one part of the system and relatively ignorant of another part, with other experts having reverse properties. Therefore, hybrid models that combine the uncertainty of multiple experts are just as plausible, or more so, than the opinion of any one expert. On the other hand one might have faith in within-expert parameter correlations in general, because if an expert is right or wrong about one component of a system they are likely to be similarly right or wrong about other aspects. The issue of within-expert correlation is not well studied. Though some others have assessed the related case of between-expert correlation in parameter estimation (e.g., \cite{Zio1997}).
...
In summary we would recommend a value of information analyses be performed to inform monitoring and even decide whether it should take place at all. We have shown that it can be used to identify which parts of a complex model system are most valuable to address. Here, given the assumptions we have outlined above, managers should focus scarce monitoring resources first on the harvest management option (if that option is on the table) and the low-density regrowth states. We caution, that these findings are dependent on the objective and the number of management options avavailable. We have highlighted this dependence by demonstrating how the value of information changes differentially among model parameters as the constraints on the decision are modified.
